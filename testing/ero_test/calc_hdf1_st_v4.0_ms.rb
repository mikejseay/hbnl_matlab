#! /usr/local/bin/ruby
#
# /opt/bin/solaris/calc_hdf1_st_v4.0.rb
#
# Niklas Manz (2014-04-17)
#
...
usage = "#{$0}\n
	...
	If the mat-file does not exist in the folder you are in, it will call calc_hdf1_st_v4.0\n
	...
	[-s threshold_electrodes <all (up to 31) default, own electrode_list>] 
...
opts = GetoptLong.new(	[ "--norm_baseline_type_to_use",	"-b",	GetoptLong::REQUIRED_ARGUMENT ],
...
logfile               = "calc_hdf1_st_v40_#{number}.log"
...
inp_filename_list.each do | inp_file_raw |
	...
	hdf_filename = inp_file_raw.strip.chomp
        ...
        system(" /export/home/nmanz/programs/bourne-shell/calc_hdf1_check_v40.sh #{hdf_filename} #{case_number}")
       	...
       	if FileTest.exist?("#{mat_files}") == TRUE then
			...
			puts "Calculating the v4.0 mat file for #{hdf_filename} with e#{elec_list_type} and case #{case_number}"

	replaceFileText("hdf_filename", 					"#{hdf_filename}",     	         "/export/home/nmanz/programs/matlab/calc_hdf1_st_v4.0.skel", "calc_hdf1_st_v40_#{number}.a")	
	replaceFileText("case_number", 					"#{case_number}", 		 			"calc_hdf1_st_v40_#{number}.a", "calc_hdf1_st_v40_#{number}.b")
	replaceFileText("elec_list_type", 				"#{elec_list_type}", 		 		"calc_hdf1_st_v40_#{number}.b", "calc_hdf1_st_v40_#{number}.a")
	replaceFileText("pre_stim_time_ms", 	      "#{pre_stim_time_ms}", 	  	 		"calc_hdf1_st_v40_#{number}.a", "calc_hdf1_st_v40_#{number}.b")
	replaceFileText("out_min_trials", 				"#{out_min_trials}", 		 		"calc_hdf1_st_v40_#{number}.b", "calc_hdf1_st_v40_#{number}.a")
	replaceFileText("out_max_trials", 				"#{out_max_trials}", 		 		"calc_hdf1_st_v40_#{number}.a", "calc_hdf1_st_v40_#{number}.b")
	replaceFileText("threshold_value", 				"#{threshold_value}", 	  	 		"calc_hdf1_st_v40_#{number}.b", "calc_hdf1_st_v40_#{number}.a")
	replaceFileText("threshold_min_time_ms", 		"#{threshold_min_time_ms}",   	"calc_hdf1_st_v40_#{number}.a", "calc_hdf1_st_v40_#{number}.b")
	replaceFileText("threshold_max_time_ms", 		"#{threshold_max_time_ms}",   	"calc_hdf1_st_v40_#{number}.b", "calc_hdf1_st_v40_#{number}.a")
	replaceFileText("the_working_directory", 		"#{the_working_directory}",   	"calc_hdf1_st_v40_#{number}.a", "calc_hdf1_st_v40_#{number}.b")
	replaceFileText("norm_baseline_type_to_use", "#{norm_baseline_type_to_use}",  "calc_hdf1_st_v40_#{number}.b", "calc_hdf1_st_v40_#{number}.a")
	replaceFileText("st_baseline_time_min_ms", 	"#{st_baseline_time_min_ms}", 	"calc_hdf1_st_v40_#{number}.a", "calc_hdf1_st_v40_#{number}.b")
	replaceFileText("st_baseline_time_max_ms", 	"#{st_baseline_time_max_ms}", 	"calc_hdf1_st_v40_#{number}.b", "calc_hdf1_st_v40_#{number}.a")
	replaceFileText("hp_filter", 						"#{hp_filter}", 		 				"calc_hdf1_st_v40_#{number}.a", "calc_hdf1_st_v40_#{number}.b")
	replaceFileText("lp_filter", 						"#{lp_filter}", 		 				"calc_hdf1_st_v40_#{number}.b", "calc_hdf1_st_v40_#{number}.a")
	replaceFileText("threshold_electrodes", 		"#{threshold_electrodes}", 	 	"calc_hdf1_st_v40_#{number}.a", "calc_hdf1_st_v40_#{number}.m")

	File.delete("calc_hdf1_st_v40_#{number}.a") if FileTest.exist?("calc_hdf1_st_v40_#{number}.a") 
	File.delete("calc_hdf1_st_v40_#{number}.b") if FileTest.exist?("calc_hdf1_st_v40_#{number}.b")

	if (use_grid.to_i == 1) then
		File.copy("#{the_working_directory}/calc_hdf1_st_v40_#{number}.m", "#{the_working_directory}/calc_hdf1_st_v40_#{number}_#{count_file}.m")
		system("grid /opt/bin/matbat10nm -- #{the_working_directory}/calc_hdf1_st_v40_#{number}_#{count_file}.m")
	else
		system("export LD_LIBRARY_PATH=/active_projects/programs/hdf1progs/lib-ibm-linux; /opt/bin/matbat10nm calc_hdf1_st_v40_#{number}.m 2>&1 > #{logfile}")
	end

	# File.delete("calc_hdf1_st_v40_#{number}.m") 		if FileTest.exist?("calc_hdf1_st_v40_#{number}.m")
	# File.delete("calc_hdf1_st_v40_#{number}.log") 	if FileTest.exist?("calc_hdf1_st_v40_#{number}.log")
	end
end
