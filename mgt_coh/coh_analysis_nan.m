...
for pair=pair_subset
    ...
    for freq=pp.chosen_freq
        for win=1:pp.maxwin
            ...
            %
            coh_scatterdata_x=squeeze(nanmean(nanmean(cohdata(t_start:t_end,freq,pp.cond_diff{1},pair,s_inds_g(:,scl.g_all)),1),3));
            coh_scatterdata_y=squeeze(nanmean(nanmean(cohdata(t_start:t_end,freq,pp.cond_diff{2},pair,s_inds_g(:,scl.g_all)),1),3));
            %test out a baseline subtraction
            %coh_scatterdata_x=squeeze(nanmean(nanmean(cohdata(t_start:t_end,freq,pp.cond_diff{1},pair,s_inds_g(:,scl.g_all)),1),3))-squeeze(nanmean(nanmean(cohdata(1:scl.t_zero,freq,pp.cond_diff{1},pair,s_inds_g(:,scl.g_all)),1),3));
            %coh_scatterdata_y=squeeze(nanmean(nanmean(cohdata(t_start:t_end,freq,pp.cond_diff{2},pair,s_inds_g(:,scl.g_all)),1),3))-squeeze(nanmean(nanmean(cohdata(1:scl.t_zero,freq,pp.cond_diff{1},pair,s_inds_g(:,scl.g_all)),1),3));
            %
...
for freq=pp.chosen_freq
    for win=1:pp.maxwin
        ...
        [~,t_end]=min(abs(scl.t_ms-pp.t_end_ms(win)));
        %
        %coh_histdata=squeeze(nanmean(nanmean(cohdata(t_start:t_end,freq,pp.cond_diff{1},pair,s_inds_g(:,scl.g_all)),1),3) - ...
        %    nanmean(nanmean(cohdata(t_start:t_end,freq,pp.cond_diff{2},pair,s_inds_g(:,scl.g_all)),1),3));
        coh_histdata=squeeze(nanmean(nanmean(nanmean(cohdata(t_start:t_end,freq,pp.cond_diff{1},seed_pairs(seed,:),s_inds_g(:,scl.g_all)),1),3),4) - ...
            nanmean(nanmean(nanmean(cohdata(t_start:t_end,freq,pp.cond_diff{2},seed_pairs(seed,:),s_inds_g(:,scl.g_all)),1),3),4));
        %coh_histdata=squeeze(...
        %    (nanmean(nanmean(cohdata(t_start:t_end,freq,pp.cond_diff{1},pair,s_inds_g(:,scl.g_all)),1),3) - nanmean(nanmean(cohdata(1:scl.t_zero,freq,pp.cond_diff{1},pair,s_inds_g(:,scl.g_all)),1),3)) - ...
        %    (nanmean(nanmean(cohdata(t_start:t_end,freq,pp.cond_diff{2},pair,s_inds_g(:,scl.g_all)),1),3) - nanmean(nanmean(cohdata(1:scl.t_zero,freq,pp.cond_diff{2},pair,s_inds_g(:,scl.g_all)),1),3)));            
        ...
        plot(ones(1,100)*nanmean(coh_histdata),linspace(pp.hist_ax(3),pp.hist_ax(4),100),'k--'); hold on;
        plot(linspace(nanmean(coh_histdata)-std(coh_histdata),nanmean(coh_histdata)+std(coh_histdata),100), ...
        ...
        z=nanmean(coh_histdata)/coh_histdata_se;
    ...
    for cond=1:imp.maxconds+1
        ...
        for freq=pp.chosen_freq
            if cond==imp.maxconds+1
                coh_plot_data = nanmean(nanmean(nanmean(cohdata(:,freq,pp.cond_diff{1},seed_pairs(seed,:),s_inds_g(:,scl.g_all)),3),4),5) - ...
        nanmean(nanmean(nanmean(cohdata(:,freq,pp.cond_diff{2},seed_pairs(seed,:),s_inds_g(:,scl.g_all)),3),4),5);
            else
                coh_plot_data=nanmean(nanmean(cohdata(:,freq,cond,seed_pairs(seed,:),s_inds_g(:,scl.g_all)),4),5);
            end
...
v=zeros(length(pp.chosen_g),imp.maxpairs,imp.maxconds+1,2);
...
for chosen_pair=pair_subset
    ...
    for cond=5:9
        ...
        if cond==imp.maxconds+1
            coh_tf_data=nanmean(atanh(cohdata(:,:,pp.cond_diff{1},chosen_pair,s_inds_g(:,group))),5) - ...
                nanmean(atanh(cohdata(:,:,pp.cond_diff{2},chosen_pair,s_inds_g(:,group))),5);
        else
            coh_tf_data=nanmean(atanh(cohdata(:,:,cond,chosen_pair,s_inds_g(:,group))),5);
        end
        contourf(fliplr(coh_tf_data)');
        colormap(pmkmp(256,pp.pmkmp_scheme))
        shading flat
        %imagesc(nanmean(cohstats(:,:,cond,chosen_pair,s_inds_g(:,group)),5)');
...
for fig=pp.figdum_init+1:pp.figdum
    ...
    for splot=1:5
        if splot==imp.maxconds+1
            subplot(3,2,splot); caxis([c_diff(1) c_diff(2)]);
        else
            subplot(3,2,splot); caxis([c(1) c(2)]);
        end
        colorbar;
    end
    tightfig;
...
for freq_range=pp.plotn_f
    ...
    if cond==imp.maxconds+1
        erpcoh_scatterdata_y=squeeze(nanmean(nanmean(nanmean(cohdata(...
            t_start:t_end,f_end:f_start,pp.cond_diff{1},pair,s_inds_g(:,group)),1),2),3)) -...
            squeeze(nanmean(nanmean(nanmean(cohdata(...
...
for freq_range=pp.plotn_f
    ...
    for win=1:length(pp.t_start_ms)
        ...
        for group=pp.chosen_g(pp.plotn_g)
        gdum=gdum+1;
        %ranova_data{gdum}=squeeze(nanmean(nanmean(cohdata(t_start:t_end,f_end:f_start,:,pair,s_inds_g(:,group)),1),2))';
        ranova_data{gdum}=inpaint_nans(squeeze(nanmean(nanmean(cohdata(t_start:t_end,f_end:f_start,:,pair,s_inds_g(:,group)),1),2)))';
        bar_data(group,:)=squeeze(nanmean(nanmean(nanmean(cohdata(t_start:t_end,f_end:f_start,:,pair,s_inds_g(:,group)),1),2),5));
        %bar_data(group,:)=squeeze(nanmean(nanmean(nanmean(cohdata(t_start:t_end,f_end:f_start,:,pair,s_inds_g(:,group)),1),2),5) -...
        %    nanmean(nanmean(nanmean(cohdata(1:scl.t_zero,f_end:f_start,:,pair,s_inds_g(:,group)),1),2),5));
        bar_data_se(group,:)=std(squeeze(nanmean(nanmean(cohdata(t_start:t_end,f_end:f_start,:,pair,s_inds_g(:,group)),1),2)),0,2)/sqrt(sum(s_inds_g(:,group)));
        %bar_data_se(group,:)=std(squeeze(nanmean(nanmean(cohdata(t_start:t_end,f_end:f_start,:,pair,s_inds_g(:,group)),1),2) -...
        %    nanmean(nanmean(cohdata(1:scl.t_zero,f_end:f_start,:,pair,s_inds_g(:,group)),1),2)),0,2)/sqrt(sum(s_inds_g(:,group)));
        %
        end
        ...
        bar_title=[];
        bar_h=nanbarweb(bar_data(pp.chosen_g,:),bar_data_se(pp.chosen_g,:),width,bar_glabel,...
...
adorn_plots(sp_rowlabel,sp_columnlabel,x_plotlabel,y_plotlabel,overtitle,[3 5]);
end
%linkaxes
%tightfig;
clear_plotassistvars
...
for chosen_pair=pair_subset
    ...
    for cond=1:imp.maxconds+1
        ...
        if cond==imp.maxconds+1
            coh_tf_data=nanmean(cohstats(:,:,pp.cond_diff{1},chosen_pair,s_inds_g(:,group)),5) - ...
                nanmean(cohstats(:,:,pp.cond_diff{2},chosen_pair,s_inds_g(:,group)),5);
        else
            coh_tf_data=nanmean(cohstats(:,:,cond,chosen_pair,s_inds_g(:,group)),5);
        end
        contourf(fliplr(coh_tf_data)');
        colormap(pmkmp(256,pp.pmkmp_scheme))
        shading flat
...
v=zeros(length(pp.chosen_g),length(seed_label),imp.maxconds+1,2);
...
for seed=1:length(seed_label)
    ...
    for cond=1:imp.maxconds+1
        ...
        %
        if cond==imp.maxconds+1
            coh_seed_avg=nanmean(nanmean(nanmean(cohdata(:,:,pp.cond_diff{1},seed_pairs(seed,:),s_inds_g(:,group)),3),4),5)-...
                nanmean(nanmean(nanmean(cohdata(:,:,pp.cond_diff{2},seed_pairs(seed,:),s_inds_g(:,group)),3),4),5);
            %coh_seed_avg=(nanmean(nanmean(nanmean(cohdata(:,:,pp.cond_diff{1},seed_pairs(seed,:),s_inds_g(:,group)),3),4),5) - ...
            %    repmat(squeeze(nanmean(nanmean(nanmean(nanmean(cohdata(1:scl.t_zero,:,pp.cond_diff{1},seed_pairs(seed,:),s_inds_g(:,group)),1),3),4),5)),imp.maxtimepts,1)) -...
            %    (nanmean(nanmean(nanmean(cohdata(:,:,pp.cond_diff{2},seed_pairs(seed,:),s_inds_g(:,group)),3),4),5) - ...
            %    repmat(squeeze(nanmean(nanmean(nanmean(nanmean(cohdata(1:scl.t_zero,:,pp.cond_diff{2},seed_pairs(seed,:),s_inds_g(:,group)),1),3),4),5)),imp.maxtimepts,1));
        else
            coh_seed_avg=nanmean(nanmean(cohdata(:,:,cond,seed_pairs(seed,:),s_inds_g(:,group)),4),5);
            %coh_seed_avg=nanmean(nanmean(cohdata(:,:,cond,seed_pairs(seed,:),s_inds_g(:,group)),4),5) - ...
                repmat(squeeze(nanmean(nanmean(nanmean(cohdata(1:scl.t_zero,:,cond,seed_pairs(seed,:),s_inds_g(:,group)),1),4),5)),imp.maxtimepts,1);
        end
        %
...
v(v==0)=NaN;
c(1)=nanmin(nanmin(nanmin(v(:,:,1:imp.maxconds,1)))); c(2)=nanmax(nanmax(nanmax(v(:,:,1:imp.maxconds,2))));
diff_cmin=nanmin(nanmin(v(:,:,imp.maxconds+1,1))); diff_cmax=nanmax(nanmax(v(:,:,imp.maxconds+1,2)));
for fig=pp.figdum_init+1:pp.figdum
    figure(fig)
    for splot=1:imp.maxconds+1
        if splot==imp.maxconds+1
            subplot(sp_d(1)+1,sp_d(2),splot); caxis([diff_cmin diff_cmax]);
        else
            subplot(sp_d(1)+1,sp_d(2),splot); caxis([c(1) c(2)]);
        end
        colorbar;
...
line_limit=[-0.06 0.16];
line_limit_diff=[-0.07 0.08];

coh_linescale_mat=zeros(imp.maxconds+1,length(pp.chosen_g),length(pp.t_start_ms),length(pp.f_start_hz),imp.maxpairs);
    ...
    for win=1:length(pp.t_start_ms)
        ...
        for pair=1:imp.maxpairs
            ...
            for statsgroup=pp.chosen_g(pp.plotn_g)
                gdum=gdum+1;
                ranova_data{gdum}=squeeze(nanmean(nanmean(cohdata(t_start:t_end,f_end:f_start,:,pair,s_inds_g(:,statsgroup)),1),2))';
            end
            ...
            if cond==imp.maxconds+1
                paircoh_for_linecolor=nanmean(nanmean(nanmean(nanmean(cohdata(t_start:t_end,f_end:f_start,pp.cond_diff{1},pair,s_inds_g(:,group)),1),2),3),5) - ...
                    nanmean(nanmean(nanmean(nanmean(cohdata(t_start:t_end,f_end:f_start,pp.cond_diff{2},pair,s_inds_g(:,group)),1),2),3),5);
                ...
                %paircoh_for_linecolor=nanmean(nanmean(nanmean(cohdata(t_start:t_end,f_end:f_start,cond,pair,s_inds_g(:,group)),1),2),5);
                paircoh_for_linecolor=nanmean(nanmean(nanmean(cohdata(t_start:t_end,f_end:f_start,cond,pair,s_inds_g(:,group)),1),2),5) -...
                    nanmean(nanmean(nanmean(cohdata(1:scl.t_zero,f_end:f_start,cond,pair,s_inds_g(:,group)),1),2),5);
                coh_linescale_mat(cond,group,win,freq_range,pair)=paircoh_for_linecolor;
...
figure(fig);
for cond=1:imp.maxconds+1
    subplot(imp.maxconds+1,length(pp.t_start_ms),cond*length(pp.t_start_ms))
    if cond==imp.maxconds+1
        colormap(pp.cmap);
...
v=zeros(imp.maxconds+1,length(pp.chosen_g),maxseeds,pp.maxwin,2);
for group=pp.chosen_g
for seed=1:maxseeds
...
for cond=1:imp.maxconds+1
    for win=1:pp.maxwin
        ...
        if cond==imp.maxconds+1
            coh_topo_data=squeeze(nanmean(nanmean(nanmean(cohdata(t_start:t_end,...
                f_start:f_end,pp.cond_diff{1},seed_pairs(seed,:),s_inds_g(:,group)),1),2),5)) -...
            squeeze(nanmean(nanmean(nanmean(cohdata(t_start:t_end,...
            ...
            coh_topo_data=squeeze(nanmean(nanmean(nanmean(cohdata(t_start:t_end,...
                f_start:f_end,cond,seed_pairs(seed,:),s_inds_g(:,group)),1),2),5) - ...
                nanmean(nanmean(nanmean(cohdata(1:scl.t_zero,...
                f_start:f_end,cond,seed_pairs(seed,:),s_inds_g(:,group)),1),2),5));
            coh_topo_data=insertrows(coh_topo_data,0,seed_inds(seed));
        end
...
for group=pp.chosen_g
chan_coh_measure=squeeze(nanmean(nanmean(cohdata(180:231,14:16,1,pair,s_inds_g(:,group)),1),2));
chan_coh_stat=squeeze(nanmean(nanmean(cohstats(180:231,14:16,1,pair,s_inds_g(:,group)),1),2));
trial_coh_measure=squeeze(nanmean(nanmean(itcdata(180:231,chosen_chan,14:16,1,s_inds_g(:,group)),1),3));
