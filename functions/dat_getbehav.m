function etable = dat_getbehav(datfile, expstruct)
...
if ischar(datfile)
    [type, rt, resp_code, n_events] = loaddat(datfile);
end
...
%get reaction times
%rt(ismember(stim_id,
nort_types = stim_id( response_id == -1 | response_id == 0);
nort_evs = ismember(type, nort_types);
rt(nort_evs) = NaN;
rt(errant==true) = NaN;

cnt_eventstruct = v2struct(event, trial, type, type_descriptor,...
    correct, rt, resp_code, errant, early, late);
