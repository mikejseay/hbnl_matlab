...
function [Y, Z, B] = pairwise_coherence_calc_noevents2(data, trial_mat, n_perms, seed, type)
	...
	Y = zeros(n_scales, n_cases);
	
	Z = []; B = [];
    if strcmpi(type,'weighted')
        for n = 1:n_cases
            trial_vec = trial_mat(:, n);
            Y(:, n) = squeeze(sum(sum( X(:, trial_vec, :),1 ), 2) ./ ...
                sum(sum(abs(X(:, trial_vec, :)),1 ), 2));
        end
    elseif strcmpi(type,'pure')
        ...
        for n = 1:n_cases
            trial_vec = trial_mat(:, n);
            Y(:, n) = squeeze(sum(sum( X(:, trial_vec, :),1 ), 2) ./ ...
                sum(sum(abs(Xdem(:, trial_vec, :)),1 ), 2));
        end
    end
