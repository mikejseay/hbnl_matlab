function [Y,name] = erp_analysis2(data_file, data_file_type, info_file, ...
...
% can use trimOutlier from eeglab
    ...
    if param_struct.cleanset
        ...
        if isfield(param_struct,'hpfilt_tbands')
            EEG = cleanset_eeglab(EEG,param_struct.hpfilt_tbands);
        else
            EEG = cleanset_eeglab(EEG); %default tbands are [0.25 0.75]
        end
