function [h1_struct] = read_hdf1_data(hdf1_filename)
%
% function [h1_struct] = read_hdf1_data(hdf1_filename)
%

hdf1_info = hdf5info(hdf1_filename);

% read relevant file info

file_info = hdf5read(hdf1_info.GroupHierarchy.Groups(1).Datasets(1));
...
if (file_struct.got_subject_info == 1)
    subject_info = hdf5read(hdf1_info.GroupHierarchy.Groups(1).Groups(3).Datasets(1));
...
if (file_struct.got_data == 1)
    if (file_struct.data_type == 0)
        ...
        hdf1_int_data = hdf5read(hdf1_info.GroupHierarchy.Groups(2).Groups(1).Datasets(1));
        data_struct.hdf1_cnt_data = double(hdf1_int_data);
        ...
        data_struct.hdf1_epoch_data = 0;
        hdf1_avg_data = hdf5read(hdf1_info.GroupHierarchy.Groups(2).Datasets(1));
        ...
        for the_case=1:size(hdf1_avg_data,3) 
            for channel=1:size(hdf1_avg_data,1) 
                for sample=1:size(hdf1_avg_data,2)
                    data_struct.hdf1_avg_data(the_case,channel,sample) = double(hdf1_avg_data(channel,sample,the_case));
                end    
            end
        end  
...
if (file_struct.got_exp_info == 1)
    experiment_info = hdf5read(hdf1_info.GroupHierarchy.Groups(1).Groups(1).Datasets(1));	
...
if (file_struct.got_run_info == 1)
    run_info = hdf5read(hdf1_info.GroupHierarchy.Groups(1).Groups(2).Datasets(1));
    ...
    if (file_struct.original_file == 1) then
...
if (file_struct.got_case_info == 1)
    case_info = hdf5read(hdf1_info.GroupHierarchy.Groups(1).Groups(2).Groups(1).Datasets(1));
...
if (file_struct.got_event_info == 1)
    event_info = hdf5read(hdf1_info.GroupHierarchy.Groups(1).Groups(2).Groups(2).Datasets(1));
...
if (file_struct.got_trial_info == 1)
    trial_info = hdf5read(hdf1_info.GroupHierarchy.Groups(1).Groups(2).Groups(3).Datasets(1));
...
if (file_struct.got_transforms_info == 1)
   transforms_info = hdf5read(hdf1_info.GroupHierarchy.Groups(1).Groups(4).Datasets(1));    
...
if (h1_struct.file_struct.data_type == 0)  
   [h1_struct] = scale_cnthdf1_data(h1_struct);
end
