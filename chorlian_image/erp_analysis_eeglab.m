function Y = erp_analysis(data_file, data_file_type, info_file, ...
	param_struct, option)
% stages
...
% stage 1
	...
	if ~isfield(param_struct, 'trial_mat')
		...
		if isfield(param_struct, 'case_vec')
...
% filter and calculate CWT
	...
	if ~isfield(param_struct, 'trial_mat')
		...
		trial_mat = check_artifact(dataR(artf_samp_vec, :, :), trial_mat, ...
			artf_chan_vec, param_struct.thresh);
	end
	...
	if option(1)
		...
		if ~isempty(coherence_pairs)
			...
			if n_perms > 0
				...
				for m = 1:n_coh_pairs
                    % this part would be switched out with a call to
                    % newcrossf
