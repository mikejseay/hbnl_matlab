function [Y,name] = erp_analysis3(data_file, data_file_type, info_file, ...
...
if isfield(param_struct,'pcheck_path')
    check_path=param_struct.pcheck_path;
else
    check_path=param_struct.output_dir;
end
...
if exist(cleanfile_path,'file')
    %load it
    load(cleanfile_path)   %should contain dataR
    %load the h1_struct
...
% can use trimOutlier from eeglab
    ...
    % clip 19 points off the end to mitigate filtering edge effects
    ...
    if param_struct.cleanset
        ...
        % if not being ASR-cleaned
        % filter data (david's function)
    ...
    % SAVE dataR here to save LOTS of time
    if isfield(param_struct, 'save_clean')
        if param_struct.save_clean
	...
	if is_mc_bin
		...
		[~, ~, dataW] = hdf_reshapeW(dataW, h1_struct, param_struct);
        %reshape scalp time-series into trials
		[~, n_samps, dataR] = hdf_reshape(dataR, h1_struct, param_struct);
...
% filter and calculate CWT
    ...
    if isfield(param_struct,'csd_G')
        ...
        if isfield(param_struct,'epochrej_method')
        if strcmpi(param_struct.epochrej_method,'simple')
            for thresh=150:50:1500
                %consider using a different epoch rejection procedure here
        ...
        elseif strcmpi(param_struct.epochrej_method,'eeglab')
            for thresh=3:.5:5
            trial_mat = check_artifact_eeglab(data_file, dataR, h1_struct, ...
                trial_mat_orig, thresh);
            prop_kept=sum(sum(trial_mat))/sum(sum(trial_mat_orig));
                if prop_kept >= .75
                    fprintf(2, 'threshold used was %d\n', thresh);
                    break
 ...
 prevoutcome_mat,avgbet2_prevoutcome,stdbet2_prevoutcome] = behav_sog(etable);
	...
	if isfield(param_struct, 'n_perms') && isfield(param_struct, 'seed')
