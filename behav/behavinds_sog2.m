...
combamount_map=containers.Map([-100 -60 -40 -20 0 20 40 60 100],[1 1 1 1 0 4 4 4 4]);
...
prevamount2_mat=zeros(1,4);
prevoutcome_mat=zeros(1,4);
...
typedescriptors={'Bet 10 Post-Loss','Bet 10 Post-Win','Bet 50 Post-Loss','Bet 50 Post-Win',...
    'Loss Trend','Even Loss','Even Gain','Gain Trend'};
n_types=length(typedescriptors);
...
for ev=1:n_events
    ...
    if ismember(etable.response_code(ev),resp_codes) && etable.errant_response(ev)==0 && ...
            ( etable.rt(ev) > rt_range(1) && etable.rt(ev) < rt_range(2))
        ...
        prevoutcome_dum=0; %counts previous outcomes
        checkprev_ev=0; %counts previous events
        prevev_tocheck=0; %indexes the event to check
        
        %for some number of previous *outcomes*
        while prevoutcome_dum<num_prev_outcomes_resp
            checkprev_ev=checkprev_ev+1;
            
            %for the first previous outcome
            if prevoutcome_dum==0
                
            %don't check before the first event
        ...
        while prevoutcome_dum<num_prev_outcomes_outcome
            ...
            if ismember(etable.type(ev - checkprev_ev),outcome_types)
                ...
                combamount = amount_map(etable.type(ev)) + ...
                    amount_map(etable.type(ev - checkprev_ev));
                combamount_ind = combamount_map(combamount);
                if combamount_ind==0
                    if amount_map(etable.type(ev)) < 0
                        combamount_ind=2;
                    else
                        combamount_ind=3;
                    end
                end
                prevoutcome_mat(combamount_ind)= ...
                    prevoutcome_mat(combamount_ind) + 1;
                
                %index!
                trial_mat(trial_dum, combamount_ind+4) = true;
...
if false %commenting this out for now
