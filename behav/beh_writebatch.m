function scriptpath = beh_writebatch(batchname,batchpath,optpath,batch_ind,start_ind,end_ind)
...
fprintf(fid,'    session = identify_cntsession_folder(data_file);\n');
fprintf(fid,'    output_file = sprintf(''%%s/%%s_%%s_%%s.mat'', output_dir, base_file, session, suffix);\n');
...
fprintf(fid,'    end\n');
fprintf(fid,'    try\n');
fprintf(fid,'        output = beh_calc(data_file, opt);\n');
fprintf(fid,'    catch err\n');
fprintf(fid,'        fprintf(fid, ''%%s: '', err.message);\n');
fprintf(fid,'        output = [];\n');
fprintf(fid,'    end\n');
fprintf(fid,'    if ~isempty(output)\n');
fprintf(fid,'        fprintf(fid, ''done\\n'');\n');
fprintf(fid,'        save(output_file, ''output'', ''-v6'');\n');
fprintf(fid,'    else\n');
