function calc_hdf1_st_v60_ms(cnthdf1_filename, working_directory, the_case, chan_list_type, baseline_type, st_baseline_time_min_ms, st_baseline_time_max_ms, min_trials, max_trials, new_pre_stim_time_ms, new_threshold_value, new_threshold_min_time_ms, new_threshold_max_time_ms, new_hi_pass, new_lo_pass, new_threshold_electrodes, response_min_time, response_max_time, trial_plot, trial_files)
%
...
if (txt_exp == 'ant')
    if (exp_case_type == 't' ) && ( the_case == 3 )
        exp_case_type = 'a';
    end
    if (exp_case_type == 't' ) && ( the_case == 1 )
        exp_case_type = 'j';
    end
    if (exp_case_type == 't' ) && ( the_case == 4 )
        exp_case_type = 'w';
    end
    if (exp_case_type == 't' ) && ( the_case == 2 )
        exp_case_type = 'p';
    end
end
if (txt_exp == 'ans')
...
if (h1_struct.experiment_struct.exp_name == 'ant')
    if (case_name == 'T' ) && ( the_case == 3 )
        case_name = 'A';
    end
    if (case_name == 'T' ) && ( the_case == 1 )
        case_name = 'J';
    end
    if (case_name == 'T' ) && ( the_case == 4 )
        case_name = 'W';
    end
    if (case_name == 'T' ) && ( the_case == 2 )
        case_name = 'P';
    end
end
...
txt_file_fname = [sub_id(1:18),'case-',case_name,'.mat.v6.0.log'];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 6.2
h1_struct = create_cnthdf1_baseline_epoch_data_v6_ms(h1_struct, txt_file_name, new_hi_pass, new_lo_pass, new_threshold_electrodes, the_case, trial_plot);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 6.3
...
fprintf(fid, 'max_window_value:');
fprintf(fid, '%10.0f', h1_struct.trial_struct.max_thresh_win_value(1:h1_struct.experiment_struct.n_trials));
fprintf(fid, '\n');
fprintf(fid, 'response_latency:');
fprintf(fid, '%5.0f', h1_struct.trial_struct.response_latency(1:h1_struct.experiment_struct.n_trials));
fprintf(fid, '\n');
% fclose(fid);
...
if (trial_files == 1 )
    ...
    for i = 1:length(case_accepted_idx)
        ...
        for k=1:length(h1_struct.run_struct.channel_label)
            fprintf(fid, '%s ', h1_struct.run_struct.channel_label{k});
        end
        fprintf(fid, '\n');
...
for i=1:n_chans
    ...
    %st_angle_cos = squeeze(sum(cos(angle(S_trial_vec))));
    %st_angle_sin = squeeze(sum(sin(angle(S_trial_vec))));
