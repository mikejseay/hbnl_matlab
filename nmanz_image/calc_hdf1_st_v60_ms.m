function calc_hdf1_st_v60_ms(cnthdf1_filename, working_directory, the_case, chan_list_type, ...
    baseline_type, st_baseline_time_min_ms, st_baseline_time_max_ms, min_trials, max_trials, ...
    new_pre_stim_time_ms, new_threshold_value, new_threshold_min_time_ms, new_threshold_max_time_ms, ...
    new_hi_pass, new_lo_pass, new_threshold_electrodes, response_min_time, response_max_time, ...
    trial_plot, trial_files)
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 6.2
h1_struct = create_cnthdf1_baseline_epoch_data_v6(h1_struct, txt_file_name, new_hi_pass, ...
    new_lo_pass, new_threshold_electrodes, the_case, trial_plot);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 6.3
...
% n_freqs must be calculated here
tmp_mean_data = zeros(1, n_samples);
[tmp_S,tmp_t,tmp_f] = stockwell_transform(tmp_mean_data, 0, length(tmp_mean_data)/2, 1/rate);
clear tmp_mean_data tmp_S tmp_t
avg_abs_S_all = zeros(length(tmp_f), n_samples, n_chans);
for i=1:n_chans
...
time_vec = (-h1_struct.experiment_struct.pre_stim_time_ms:(1000/rate): ...
    h1_struct.experiment_struct.post_stim_time_ms)';
time_vec_ds = time_vec(1:time_ds_factor:end);

avg_abs_S_all_ds = avg_abs_S_all(f_ds_vec, 1:time_ds_factor:end, :);

data = avg_abs_S_all_ds;
% data = avg_abs_S_all;
opt = v2struct(cnthdf1_filename, working_directory, the_case, chan_list_type, ...
    baseline_type, st_baseline_time_min_ms, st_baseline_time_max_ms, min_trials, ...
    max_trials, new_pre_stim_time_ms, new_threshold_value, new_threshold_min_time_ms, ...
    new_threshold_max_time_ms, new_hi_pass, new_lo_pass, new_threshold_electrodes, ...
    response_min_time, response_max_time, trial_plot, trial_files, ...
    f_ds_vec, f_vec, f_vec_ds, time_ds_factor, time_vec, time_vec_ds);
outname = strjoin({subject_id, subject_session_run(1), txt_exp, exp_case_type}, '_');
save([outname,'ds.mat'], 'data', 'opt', '-v7.3');
